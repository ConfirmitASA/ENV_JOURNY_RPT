    ...
    static function buildKPITiles (context) {
        ...
        for (var i=0; i<kpiResults.length; i++) {
            ...
            if (state.ReportExecutionMode == ReportExecutionMode.ExcelExport) {
                ...
                var content = {
                    header: '<span class="card__title card__title_position_center">' + TextAndParameterUtil.getTextTranslationByKey(context, 'Page_KPI_Trend.trendTitle') + '</span><h2 class="card__subtitle card__subtitle_position_center">' + kpiResults[i].title + '</h2>',
