class StyleAndJavaScriptUtil {
    ...
    * Assemble all "backend dependant" css styles and js scripts
    ...
    static function buildReportTemplateModule (context) {
        ...
        if (pageContext.Items['CurrentPageId'] === 'Comments' || pageContext.Items['CurrentPageId'] === 'Wordclouds') {
        ...
        properties.push('exportWindowOptionFlags: {pdf: ' + JSON.stringify(Config.pdfExportSettingsOptions.flags));
        properties.push('excel: ' + JSON.stringify(Config.excelExportSettingsOptions.flags));
        properties.push('ppt: ' + JSON.stringify(Config.pptExportSettingsOptions.flags)+'}');

        properties.push('exportWindowOptionIndexes: {pdf: ' + JSON.stringify(Config.pdfExportSettingsOptions.indexes));
        properties.push('excel: ' + JSON.stringify(Config.excelExportSettingsOptions.indexes));
        properties.push('ppt: ' + JSON.stringify(Config.pptExportSettingsOptions.indexes)+'}');
    ...
    static function applyTheme(context) {
        ...
        */
        ...
        // Select top N and then "not" to select all the others and hide
        if(numberOfVerbatimComments) {
            numberOfVerbatimComments += 1;
            css_string += '.card__verbatim-container tbody tr:not(:nth-child(-n+' + numberOfVerbatimComments + ')) { display: none; }';
        }
