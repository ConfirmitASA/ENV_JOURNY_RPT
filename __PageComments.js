     ...
     /**
       ...
       static function hitlistComments_Render(context){
         ...
         for (var i=0; i<staticCols.length; i++) {
           Hitlist.AddColumn(context, staticCols[i], {sortable: true, searchable: false});
         }   
        ...
        * @param {Object} context - {pageContext: this.pageContext, report: report, user: user, state: state, confirmit: confirmit, log: log}
        * @return {Array} - array with the numbers of columns
        */
       static function getTagColumnNumbers (context) {
         ...
         var numberOfColumnsAtStart = 3; // Hitlist always contains 1 verbatim + 2 first hidden columns with system fields: Respondent ID and Survey Id. 
         ...
         return tagColumnNumbers.sort().reverse();  // descending order of column numbers to remove them from table more easily
       }
     
     
      /**
        * @memberof Page_comments
        * @function getNonTagColumnsCount
        * @description function to get the total number of non-tag columns
        * @param {Object} context - {pageContext: this.pageContext, report: report, user: user, state: state, confirmit: confirmit, log: log}
        * @return {Int} - the total number of non-tag columns
        */
       static function getNonTagColumnsCount (context) {
         ...
         var numberOfColumnsAtStart = 3; // Hitlist always contains 1 verbatim + 2 first hidden columns with system fields: Respondent ID and Survey Id. 
         var numberOfStaticCols = DataSourceUtil.getPagePropertyValueFromConfig (context, pageId, 'staticColumns').length;
         
         return numberOfColumnsAtStart + numberOfStaticCols;      
